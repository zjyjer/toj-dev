...
/*
   TOJ hasn't login page, login when submit.
*/
...
bool submit(string pid,string lang,string source) {
	...
	if(curl) {
		...
		curl_easy_setopt(curl, CURLOPT_COOKIEFILE, "toj.cookie");
		curl_easy_setopt(curl, CURLOPT_URL, "http://acm.tju.edu.cn/toj/submit_process.php");
		/*
		 */

		string post=(string)"user_id="+username+"&passwd="+password+"&prob_id="+pid+"&language="+lang+"&source="+escapeURL(source);
		//cout<<post;
	...
	//cout<<ts;
	if (ts.find("Internal Error")!=string::npos||ts.find("You submitted too quick")!=string::npos) return false;
	return true;
}

string getResult(string s) {
	if (s.find("<font color=") == string::npos) {
		if (s.find("Compilation Error") == string::npos) return "Restricted Function";
		return "Compilation Error";
	} else {
		if (s.find("<font color=red>") == string::npos) {
			int pos1 = s.find("<font color=green><b>") + strlen("<font color=green><b>");
			int pos2 = pos1;
			while(s[pos2] != '<') pos2 ++;
...
string getCEinfo(string runid) {
	...
	if(curl) {
		...
		string url=(string)"http://acm.tju.edu.cn/toj/show_cerror.php?id="+runid;
		//cout<<url;
...
string getUsedTime(string s) {
	int pos = s.find("</td><td>") + strlen("</td><td>");
	int tu = ((s[pos+2]-'0')*10+(s[pos+3]-'0')) * 1000 + ((s[pos+5]-'0')*10+(s[pos+6]-'0')) * 10;
	return covert(tu);
}

string getUsedMem(string s) {
	int pos = s.find("K</td>");
	return s.substr(4, pos - 4);
}
...
bool getStatus(string pid,string lang,string & result,string& ce_info,string &tu,string &mu) {
	...
	while (true) {
		...
		if(curl) {
			curl_easy_setopt(curl, CURLOPT_WRITEDATA, fp);
			curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, NULL);
			curl_easy_setopt(curl, CURLOPT_COOKIEJAR, "toj.cookie");
			string url=(string)"http://acm.tju.edu.cn/toj/status.php?pid="+pid+"&user="+username;
			//cout<<url;
			curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
			res = curl_easy_perform(curl);
			curl_easy_cleanup(curl);
		...
		ts=getLineFromFile(tfilename,19);
		if (ts == "") return false;
		runid=getRunid(ts);

		ts=getLineFromFile(tfilename,20);
		if (ts == "") return false;
		result=ts;


		if (result.find("Received")==string::npos
	...
	if (result=="Accepted") {
		ts=getLineFromFile(tfilename,22);
		tu=getUsedTime(ts);
		ts=getLineFromFile(tfilename,23);
		mu=getUsedMem(ts);
