...
exports.newAndSave = function(doc, callback) {
	...
	_topic.cid = doc.cid;
	_topic.pid = doc.pid;
	_topic.title = doc.title;
	_topic.content = doc.content;
	_topic.author = doc.username;
...
exports.getTopicById = function (id, callback) {
	...
	Topic.findOne({_id: id}, proxy.done(function (topic) {
		...
		User.getByName({ username: topic.author }, proxy.done('author'));
...
exports.getTopicsByQuery = function (query, opt, callback) {
	Topic.find(query, { _id: 1 }, opt, function (err, docs) {
		if (err) {
			return callback(err);
		}
		if (docs.length === 0) {
...
exports.getFullTopic = function (id, callback) {
	...
	Topic.findOne({_id: id}, proxy.done(function (topic) {
		if (!topic) {
			proxy.unbind();
			return callback(null, 'The topic does not exists.');
		}
		proxy.emit('topic', topic);

		User.getByName({ username: topic.author }, proxy.done(function (user) {
			if (!user) {
				proxy.unbind();
				return callback(null, 'The author is lost.');
			}
